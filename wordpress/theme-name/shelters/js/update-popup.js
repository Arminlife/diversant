...
(function($) {
  ...
  async function submitReview(formData) {
    try {
      const response = await fetch('https://slidstvo.wstart.com.ua/wp-json/shelters/v1/review', {
        method: 'POST',
        headers: {
  ...
  function updateMarkerPopup(shelterData) {
    ...
    const infoItems = [
      ...
      { icon: 'accessibility-sign', label: `Інклюзивність: ${shelterData.inclusive ? 'Так' : 'Ні'}` },
      { icon: 'clock', label: `Режим роботи: ${shelterData.fullday || 'Не вказано'}` },
      { icon: 'hammer-outline', label: `Стан: ${shelterData.condition || 'Не вказано'}` },
      { icon: 'sofa', label: `Місце для сидіння: ${shelterData.seats ? 'Так' : 'Ні'}` },
      { icon: 'badge-wc', label: `Туалет: ${shelterData.wc ? 'Так' : 'Ні'}` },
      { icon: 'water tap', label: `Вода: ${shelterData.water ? 'Так' : 'Ні'}` },
      { icon: 'number', label: `Місткість: ${shelterData.capacity || 'Не вказано'}` },
      { icon: 'area-search', label: `Площа: ${shelterData.square || shelterData.area || 'Не вказано'}` }
    ];
...
// Update gallery
    ...
    } else if(shelterData.destroyed != null) {
      $popup.find('.gallery_v2').remove();
  ...
  window.updateShelterPopupData = function(shelterId) {
    ...
    fetchSheltersData().then(data => {
      ...
      if (!data) {
        console.error('Failed to fetch shelters data');
        return;
      }
  ...
  $(document).ready(function() {
    console.log('Shelter map popup script loaded');
    console.log('shelterMapPopupData available:', typeof shelterMapPopupData !== 'undefined');

    if (typeof shelterMapPopupData !== 'undefined') {
      console.log('Theme URL:', shelterMapPopupData.themeUrl);
    } else {
      console.error('shelterMapPopupData is not defined! Make sure wp_localize_script is working.');
    }

    console.log('Popup element exists:', $('.js-map-marker-popup').length > 0);

...
class Utils {
	 ...
	 * usage: Utils::icon( 'check' ); or Utils::icon( 'check', 'test_mod' );
	 *
	 * @param $icon_name
	 * @param null $icon_mod
	 *
	...
	static function the_icon( $icon_name, $icon_mod = null ) {
		if ( $icon_name ) {
			...
			$out    .= '<svg class="' . $classes . '"><use xlink:href="' . get_template_directory_uri() . '/assets/images/sprite/sprite.svg#' . $icon_name . '"></use></svg>';
			...
			foreach ( $array_menus as $key => $array_menu ) {
				if ( empty( $array_menu->menu_item_parent ) ) {
					...
					$menu[$current_id] = array(
						...
						'icon_name'  => get_field('menu_icon', $current_id),
						'divider'    => get_field('menu_divider', $current_id),
						'sub_menu'   => array(),
						'classes'    => (untrailingslashit($array_menu->url) == untrailingslashit($current_url)) ? 'header_nav__item--active_state' : ''
					);
				}

				if ( isset( $current_id ) && $current_id == $array_menu->menu_item_parent ) {
					$submenu_id = $array_menu->ID;
					$picture = get_field( 'menu_icon', $submenu_id );
	...
	public static function the_picture( $source, $attr = null ) {
		if ( ! empty( $source ) ) {
			...
			if ( ! empty( $attr ) && array_key_exists( 'lazy', $attr ) ) {
				$lazy = 'loading="' . $attr['lazy'] . '"';
			} else {
				$lazy = '';
			}
			...
			$img_url = !empty($source['desktop']['image']['url']) ? $source['desktop']['image']['url'] : ($source['desktop']['image_webp'] ?? '');
			$alt_text = !empty($source['desktop']['image']['alt']) ? $source['desktop']['image']['alt'] : ( $source['desktop']['image']['alt'] ?? '');

			$out .= '<img ' . $img_class . ' src="' . esc_url($img_url) . '" alt="' . esc_attr($alt_text) . '" ' . $lazy . ' />';
			$out .= '</picture>';

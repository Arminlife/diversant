<?php
	global $post;

	$primary_tag = get_term(get_post_meta($post->ID, '_yoast_wpseo_primary_tags')[0]);
	$related_posts = (new WP_Query([
        'post_type' => ['articles', 'news', 'blogs', 'warnews', 'english-stories'],
        'offset' => '1',
        'post__not_in' => [$post->ID],
        'tax_query' => [
                [
                    "taxonomy" => 'tags',
                    "field" => "term_id",
                    "terms" => $primary_tag->term_id
            ]
        ],
    ]))->get_posts();
?>

	<?php if ( ! empty( $related_posts ) && is_array( $related_posts ) ) : ?>

	<?php $related_posts = array_slice($related_posts, 0, 3); ?>

	<div class="related-posts">

		<div class="container-fluid">

			<h2><?php esc_html_e( 'Related posts', 'slidstvo-info-theme' ); ?></h2>

			<div class="row">

				<?php foreach ( $related_posts as $related_post ) : ?>

					<article class="col-lg-4 col-md-6">

						<a href="<?= get_post_permalink( $related_post->ID ); ?>" title="<?= get_the_title( $related_post ); ?>" class="image_link">

							<div class="image">
								<?php echo get_the_post_thumbnail( $related_post->ID, 'thumbnails-vertical' ); ?>
								<h2><?php echo get_the_title( $related_post ); ?></h2>
							</div>


							<p class="excerpt"><?php echo the_excerpt_max_charlength( 120, $related_post->ID ); ?></p>

						</a>

					</article>

			<?php endforeach; ?>

		</div>

	</div>

<?php endif; ?>

...
// require_once( 'classes/class.warnews-functions.php' );
...
function slidstvo_info_styles() {
    ...
    wp_register_style( 'owl-theme', get_template_directory_uri() . '/dist/css/owl.theme.default.min.css'  );
    wp_enqueue_style( 'owl-theme' );
	wp_register_style( 'percircle', get_template_directory_uri() . '/dist/css/percircle.css'  );	wp_enqueue_style( 'percircle' );

	wp_register_style( 'childstyle', get_template_directory_uri() . '/assets/css/main.min.css?v=77777'  );
...
function slidstvo_info_scripts(){
	...
	wp_register_script( 'map-js', get_template_directory_uri() . '/dist/js/map.js', '' , 'null', true );
	wp_enqueue_script( 'map-js' , 'jquery' );
	wp_register_script( 'percircle', get_template_directory_uri() . '/dist/js/percircle.js' , '' , 'null', true);
    ...
    wp_register_script( 'bootstrap-js', get_template_directory_uri() . '/dist/js/bootstrap.js', '' , 'null', true);
    wp_enqueue_script( 'bootstrap-js' , 'jquery' );


    wp_enqueue_script('main.js', get_template_directory_uri() . '/dist/js/main.js', array(jquery), filemtime(get_template_directory() . '/dist/js/main.js'), true);
    wp_enqueue_script( 'main.js' , 'jquery' );
...
function include_post_types_in_feed($qv) {
    if (isset($qv['feed']) && !isset($qv['post_type']))
        $qv['post_type'] = array('animation', 'video', 'articles', 'news' , 'special-projects', 'blogs', 'warnews', 'english-stories');
    return $qv;
}
                ...
                <item>
                    ...
                    <?php else : ?>
                        ...
                        <?php if ( strlen( $content ) > 0 ) : ?>
                            <fulltext><![CDATA[<?php echo $content; ?>]]></fulltext>
                        <?php else : ?>
...
add_action( 'wp_head', 'ozz_english_stories_black_button' );
...
add_action( 'wp_head', 'ozz_slidctvo_club_black_button' );
...
add_action( 'wp_head', 'ozz_warnews_red_button' );
...
// temporary code to implement Special Projects menu item in test mode
add_action( 'wp_head', 'ozz_test_spec_menu_item' );
function ozz_test_spec_menu_item() {
	?>
	<style>
		ul.spec_sub_menu {
			list-style: none;
...
add_action( 'wp_head', 'ozz_wider_container' );
...
// make both news columns on main page of equal height
	...
	if ( is_front_page() ) {
		...
		<script>
			jQuery(window).load(function() {
				setTimeout(function() {
					const leftColumn		= jQuery('.news-block-main-news');
					const rightColumn		= jQuery('.news-block-articles');
					const rightColumnHeight	= jQuery('.news-block-articles-list').outerHeight(true) + jQuery('.news-block-articles').children('.more-news').height() + 10;
					
					// define which height is bigger
					var shortColumn	= leftColumn;
					var longColumn	= rightColumn;
					var normHeight	= rightColumnHeight;
					if (leftColumn.height() > rightColumnHeight) {
						shortColumn		= rightColumn;
						longColumn		= leftColumn;
						var normHeight	= leftColumn.height();
					} else if (leftColumn.height() == rightColumnHeight) {
						return false; // they're equal, nothing to do here
					}

					// define how many children there are in shorty
					const shortyChildren = shortColumn.find('article').length
					
					// define height difference
					const heightDiff = Math.abs(leftColumn.height() - rightColumnHeight);
					...
					const newMarginTop = parseInt(shortColumn.find('article').css('margin-top')) + singleDiff;
					
					// equalize columns
					shortColumn.find('article').not(':first').css('margin-top', newMarginTop+'px');

					// also equalize right (video) column
					const videoBlockHeight		= jQuery('.news-block-right-col .news-video-block').height();
					const videoHeightDiff		= normHeight - videoBlockHeight;
					const videoBannerImgHeight	= jQuery('.news-block-right-col .news-video-block').find('.banner img').height();
					jQuery('.news-block-right-col .news-video-block').find('.banner img').height(videoBannerImgHeight + videoHeightDiff);
				}, 500);
			});
		</script>
		<?php
...
add_action( 'wp_head', 'ozz_remove_nofollow_from_social' );

...
export const onWindowResize = (cb) => {
	...
	window.addEventListener('resize', debounce(handleResize, 15));
...
export const isLowEndDevice = () => {
	...
	const isOldIOS = isIOS && /OS [5-9]_/.test(window.navigator.userAgent);
	const lowMemory = window.navigator.deviceMemory && window.navigator.deviceMemory < 4;
	const lowCores = window.navigator.hardwareConcurrency && window.navigator.hardwareConcurrency < 4;

	return isOldIOS || lowMemory || lowCores;
};
